...
NetworkManager::NetworkManager(FancyLog& fancyLog, OTAManager& otaManager, DisplayManager& display)
    : fancyLog(fancyLog), otaManager(otaManager), display(display), eepromManager(nullptr), updateAvailable(false) {}

void NetworkManager::setEEPROMManager(EEPROMManager* manager) {
...
void NetworkManager::saveWiFiCredentials(const char* ssid, const char* password) {
    if (eepromManager != nullptr) {
        ...
        fancyLog.toSerial("Cannot save WiFi credentials - no EEPROM manager available", WARNING);
    }
}
...
bool NetworkManager::loadWiFiCredentials(char* ssid, char* password) {
    if (eepromManager != nullptr) {
        return eepromManager->loadWiFiCredentials(ssid, password);
    } else {
        fancyLog.toSerial("Cannot load WiFi credentials - no EEPROM manager available", WARNING);
        return false;
    }
}
...
bool NetworkManager::hasStoredCredentials() {
    if (eepromManager != nullptr) {
        return eepromManager->hasStoredWiFiCredentials();
    }
    return false;
}
