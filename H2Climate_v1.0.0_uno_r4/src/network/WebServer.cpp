...
WebServer::WebServer(FancyLog& log, NetworkManager& network)
    : fancyLog(log), networkManager(network), eepromManager(nullptr), server(80) {}

void WebServer::setEEPROMManager(EEPROMManager* manager) {
...
void WebServer::saveNetworkCredentials(const char* ssid, const char* password) {
    ...
    if (eepromManager != nullptr) {
        // Use the EEPROM manager to save credentials
        eepromManager->saveWiFiCredentials(ssid, password);
        fancyLog.toSerial("Network credentials saved to EEPROM", INFO);
    } else {
        // No EEPROM manager available
        fancyLog.toSerial("Cannot save network credentials - no EEPROM manager available", WARNING);
    }
